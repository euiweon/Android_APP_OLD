...
public class TravelActivity2 extends HotelJoinBaseActivity implements OnClickListener{
	TravelActivity2  self;
	 SlidingMenu menu ;
	 int MenuSize;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.travel_main2);
	...
	public void onClick(View v) {
		...
		case R.id.write:
		{
			...
			{
				Intent intent;
	            intent = new Intent().setClass(baseself, TravelWrite2Activity.class);
		...
		case R.id.mylist:
			...
			{
				Intent intent;
	            intent = new Intent().setClass(baseself, MyTravel2Activity.class);
	            startActivity( intent ); 
			}
			else
			{
				...
				try 
				{
					...
					if(json.getString("errorCode").equals("0"))
					{
						...
						for(int i = 0; i < usageList.length(); i++)
						{
							...
							// 어차피 호텔코드는 도시와 국가가 변경될리 없으므로
							// 호텔코드로 검색을 했으면 그냥 넣어주자 
