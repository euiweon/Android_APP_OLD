...
public class UISizeConverter {
	...
	public void ConvertFrameLayoutTextView( TextView image )
	{
		...
		float size = image.getTextSize();
		size = ( (float) size * ( (float)(windowsheight)/ (float)(originheight) )  );
		//image.setTextSize(size);
		image.setTextSize(TypedValue.COMPLEX_UNIT_PX, size );
		FrameLayout.LayoutParams margin = (FrameLayout.LayoutParams )(image.getLayoutParams());
	...
	public void ConvertRelativeLayoutTextView( TextView image )
	{
		if ( bResize == false )
    		return;
    	Display defaultDisplay = ((WindowManager)m_Context.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
	...
	public void ConvertLinearLayoutTextView( TextView image )
	{
		if ( bResize == false )
    		return;
    	Display defaultDisplay = ((WindowManager)m_Context.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
	...
	// 리스트뷰의 ConvertView에만 적용해야한다. 
	// 하위 뷰는 부모의 영향을 받으므로 상관 없을듯....
	public void ConvertListViewSize( View image )
	{
		if ( bResize == false )
    		return;
    	Display defaultDisplay = ((WindowManager)m_Context.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
		...
		android.widget.AbsListView.LayoutParams margin = (android.widget.AbsListView.LayoutParams )(image.getLayoutParams());
		image.getLayoutParams().width = (int) ( (float) imagewidth * ( (float)(windowswidth)/ (float)(originwidth) )  );
		image.getLayoutParams().height = (int) ( (float) imageheight * ( (float)(windowsheight)/ (float)(originheight) )  );
	}
