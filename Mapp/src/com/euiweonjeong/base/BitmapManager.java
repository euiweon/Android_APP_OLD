...
public enum BitmapManager {
	...
	private Map<ImageView, String> imageViews2 = Collections
			.synchronizedMap(new WeakHashMap<ImageView, String>());
	...
	public void loadBitmap2(final String url, final ImageView imageView,
		...
		Bitmap bitmap = getBitmapFromCache2(url);
		imageView.setAdjustViewBounds(true);
		// check in UI thread, so no concurrency issues
		if (bitmap != null) {
			Log.d(null, "Item loaded from cache2: " + url);
			imageView.setImageBitmap(bitmap);
		} else {
			imageView.setAdjustViewBounds(true);
			imageView.setImageBitmap(placeholder);
	...
	private Bitmap downloadBitmap2(String url, int width, int height) {
		try {
			...
			bitmap = Bitmap.createScaledBitmap(bitmap, width, height, true);
			cache2.put(url, new SoftReference<Bitmap>(bitmap));
			return bitmap;
		} catch (MalformedURLException e) {
			e.printStackTrace();
		} catch (IOException e) {
